...
import torch
from torchtrainer.callbacks import CheckpointIteration
from torchtrainer.callbacks.callbacks import Callback


class CustomCheckpointIteration(Callback):
    def __init__(self, directory, used_user_embedding_path, config, filename='snapshot', best_filename='best',
                 monitor='val_loss', ):
        """
        ...
        self.best = float('inf')
        self.used_user_embedding_path = used_user_embedding_path
        self.config = config

        super(CustomCheckpointIteration, self).__init__()
    ...
    def save(self, iteration, file, is_best=False):
        torch.save({
            ...
            'used_embedding_path': self.used_user_embedding_path,
...
class CustomCheckpointEpoch(Callback):
    def __init__(self, directory, used_user_embedding_path, config, filename='snapshot', best_filename='best',
                 monitor='val_loss', ):
        """
        ...
        super(CustomCheckpointEpoch, self).__init__()
...
class CustomCheckpointIterationPlain(CheckpointIteration):
    def on_iteration(self, iteration, logs=None):
        logs = logs or {}
        values = f'epoch_{iteration + 1}_'
        for key, item in logs.items():
